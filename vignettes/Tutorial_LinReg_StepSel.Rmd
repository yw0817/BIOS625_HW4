...
```{r}
library(MASS)
result = bench::mark(stepAIC(full_lm, direction = "backward", k = log(n), trace = FALSE)$coefficients, StepSelect(full_lm, direction = "backward", k = log(n), trace = FALSE)$coefficients, )
print(result)
```
...
```{r}
result = bench::mark(stepAIC(m, direction="forward", k = log(n), scope=list(lower=m, upper=full_lm), trace = FALSE)$coefficients, my2 = StepSelect(full_lm, direction = "forward", k = log(n), trace = FALSE)$coefficients)
print(result)
```
